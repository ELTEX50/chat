...
window.editMessage = function(button) {
    ...
    // Check if this is an AI message (which cannot be edited)
    if (message.classList.contains('ai-message')) {
        if (window.showToast) {
            window.showToast('AI messages cannot be edited', 'warning');
        }
        return;
    }
    ...
    if (messageInput) {
        ...
        window.pimxchat.editingMessageId = message.dataset.messageId;
        window.pimxchat.originalMessageText = messageText;
        ...
        // Update send button to show "Update" instead of "Send"
        ...
        // Add visual indication that we're editing
        ...
        if (window.showToast) {
            window.showToast('Editing message...', 'info');
        }
    }
};

// Global regenerate message function
...
class PIMXCHAT {
    ...
    cancelEdit() {
        if (this.editingMessageId) {
            this.exitEditMode();
            if (window.showToast) {
                window.showToast('Edit cancelled', 'info');
            }
        }
    }
    ...
    initParticleSystem() {
        ...
        // Initialize dynamic waves
        this.initDynamicWaves();
    ...
    initCursorTrail() {
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            // Create trail dot
            this.createTrailDot(mouseX, mouseY, cursorTrail);
        });

        // Create periodic trail dots for automatic animation
        setInterval(() => {
            if (Math.random() > 0.7) {
        const x = Math.random() * window.innerWidth;
        const y = Math.random() * window.innerHeight;
                this.createTrailDot(x, y, cursorTrail);
            }
        }, 100);
    }

    createTrailDot(x, y, container) {
    ...
    createParticleExplosion(particle) {
        ...
        explosion.style.background = 'radial-gradient(circle, var(--neon-purple) 0%, transparent 70%)';
    ...
    initDynamicWaves() {
        ...
        waves.forEach((wave, index) => {
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                wave.style.transform = `rotate(${mouseX * 360}deg) scale(${1 + mouseY * 0.1})`;
            });
        });
    }
    ...
    initEnergyPulses() {
        ...
        // Create multiple pulse layers
        for (let i = 0; i < 3; i++) {
            const pulse = document.createElement('div');
            pulse.style.position = 'absolute';
            pulse.style.width = '100%';
            pulse.style.height = '100%';
            pulse.style.background = 'radial-gradient(circle, var(--neon-purple) 0%, transparent 70%)';
            pulse.style.animation = `energy-pulse ${8 + i * 2}s ease-in-out infinite`;
            pulse.style.animationDelay = i * 2 + 's';
            pulse.style.opacity = '0.02';
            
            energyPulse.appendChild(pulse);
        }
    }
    ...
    addMessageAnimation(messageElement) {
        ...
        ripple.style.background = 'radial-gradient(circle, var(--neon-purple) 0%, transparent 70%)';
    ...
    createFloatingText(text, x, y) {
        ...
        floatingText.style.color = 'var(--neon-purple)';
        floatingText.style.fontSize = '12px';
        floatingText.style.fontWeight = 'bold';
        floatingText.style.pointerEvents = 'none';
        floatingText.style.zIndex = '1000';
        floatingText.style.animation = 'floating-text 2s ease-out forwards';
    ...
    async sendMessage() {
        ...
        const messageElement = this.addMessageToUI(message, 'user');
        this.playSound('message');

        // Add animation effects
        this.addMessageAnimation(messageElement);
        this.createFloatingText('✨', Math.random() * window.innerWidth, Math.random() * window.innerHeight);

        // Hide Think Harder button when user sends a message
        this.hideThinkHarderButton();
        ...
        try {
            ...
            if (response.success) {
                ...
                // Add AI response with fast typing animation
                console.log('Adding AI response to UI:', response.ai_message.content);
                const aiMessageElement = await this.addAIResponseWithTyping(response.ai_message.content);
                this.playSound('notification');
                
                // Add special animation for AI response
                this.addMessageAnimation(aiMessageElement);
                this.createFloatingText('🤖', Math.random() * window.innerWidth, Math.random() * window.innerHeight);
            ...
            this.hideTypingIndicator();
            const errorMessageElement = this.addMessageToUI('متأسفم، خطایی رخ داد. لطفاً دوباره تلاش کنید.', 'ai');
            this.addMessageAnimation(errorMessageElement);
            this.playSound('error');
        }
    }

    // Add AI response with fast typing animation
    async addAIResponseWithTyping(content) {
        ...
        // Grok-style message actions for AI
        ...
        await this.typeText(content, `typing-text-${messageId}`);
        
        return messageDiv;
    }

    // Fast typing animation
    async typeText(text, elementId, speed = 30) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (i % 3 === 0) { // Type every 3rd character with delay for faster effect
    ...
    async updateMessage() {
        ...
        try {
            ...
            const messageBubble = messageElement.querySelector('.message-bubble p');
            if (messageBubble) {
                messageBubble.innerHTML = this.parseMarkdown(this.escapeHtml(message));
            }

                    // Add "edited" indicator
        const messageTime = messageElement.querySelector('.message-time');
        if (messageTime) {
            // Check if already has edited indicator
            if (!messageTime.querySelector('.edited-indicator')) {
                messageTime.innerHTML = `${messageTime.textContent} <span class="edited-indicator">(edited)</span>`;
            }
        }
            ...
            if (window.showToast) {
                window.showToast('Message updated successfully', 'success');
            }
    ...
    exitEditMode() {
        ...
        // Remove editing class from all messages
        const editingMessages = document.querySelectorAll('.message.editing');
        editingMessages.forEach(msg => msg.classList.remove('editing'));
    ...
    addMessageToUI(content, type) {
        ...
        // No avatars in Grok-style layout
        const avatar = '';
        ...
        ` : `
            <div class="message-actions">
                ...
                <button class="message-action-btn edit-btn" onclick="editMessage(this)" title="ویرایش پیام">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        // Show "Think Harder" button for AI messages after a delay
        if (type === 'ai') {
            setTimeout(() => {
                this.showThinkHarderButton();
            }, 2000);
        }
    ...
    showThinkHarderButton() {
        ...
        if (container && !this.thinkHarderMode) {
            container.style.display = 'block';
        }
    }

    hideThinkHarderButton() {
        const container = document.getElementById('thinkHarderContainer');
        if (container) {
    ...
    async createNewChat() {
        try {
            ...
            if (window.showToast) {
                window.showToast('Error creating new chat', 'error');
            }
        }
    }
    ...
    async deleteSession(sessionId) {
        ...
        if (window.showToast) {
            window.showToast('Deleting chat...', 'info');
        }
        ...
        const saveRename = async () => {
            ...
            try {
                ...
                if (response.success) {
                    ...
                    this.playSound('notification');
                    if (window.showToast) {
                        window.showToast('Chat name updated successfully', 'success');
                    }
                } else {
                    if (window.showToast) {
                        window.showToast('Error updating chat name', 'error');
                    }
                    this.cancelRename(titleElement, input);
                }
            } catch (error) {
    ...
    async clearCurrentChat() {
        ...
        // Show confirmation toast
        if (window.showToast) {
            window.showToast('Clearing chat...', 'info');
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                this.playSound('notification');
                if (window.showToast) {
                    window.showToast('Chat cleared successfully', 'success');
                }
            } else {
                if (window.showToast) {
                    window.showToast('Error clearing chat', 'error');
                }
            }
        } catch (error) {
    ...
    exportChat() {
        if (!this.currentSessionId) {
            if (window.showToast) {
                window.showToast('No chat to export', 'warning');
            }
            return;
        }
        ...
        if (window.showToast) {
            window.showToast('Chat exported successfully', 'success');
        }
    }
    ...
    initKeyboardEffects() {
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Enter' || e.key === ' ') {
                this.createKeyboardRipple(e.clientX, e.clientY);
            }
        });
    }

    createKeyboardRipple(x, y) {
        const ripple = document.createElement('div');
